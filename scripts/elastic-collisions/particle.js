  ...
  collide(other) {
    ...
    */
    ...
    if (distance < this.radius + other.radius) {      
      ...
      const velDiff = {x: other.vel.x - this.vel.x, y: other.vel.y - this.vel.y};
      const dot = velDiff.x * impact.x + velDiff.y * impact.y;
      // This particle
      const den = mSum * distance * distance;
      const deltaVA = {x: impact.x * (2 * other.mass * dot / den), y: impact.y * (2 * other.mass * dot / den)};
      this.vel.x += deltaVA.x;
      this.vel.y += deltaVA.y;
      // Other particle
      const deltaVB = {x: impact.x * (-2 * this.mass * dot / den), y: impact.y * (-2 *this.mass * dot / den)};
